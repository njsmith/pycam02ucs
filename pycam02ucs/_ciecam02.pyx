...
cdef void JCh_to_XYZ_inner(char ** args, np.npy_intp * dimensions,
                           np.npy_intp * steps, void * data):
    cdef np.npy_intp i
    cdef char * J_arr = args[0]
    cdef np.npy_intp J_steps = steps[0]
    cdef char * C_arr = args[1]
    cdef np.npy_intp C_steps = steps[1]
...
cdef np.PyUFuncGenericFunction * JCh_to_XYZ_loops = [&JCh_to_XYZ_inner]
cdef char * JCh_to_XYZ_types = [np.NPY_DOUBLE, np.NPY_DOUBLE, np.NPY_DOUBLE,
                                np.NPY_DOUBLE, np.NPY_DOUBLE, np.NPY_DOUBLE]
cdef void ** JCh_to_XYZ_data = [NULL]

cdef char * JCh_to_XYZ_docstring = (
"""Calculate XYZ values given CIECAC02 J, C, h values.

J = lightness, C = chroma, h = hue angle (0-360).

Assumes the CIECAM02 "average" surround, with D65 whitepoint, 20% gray, and
"La value of 4 cd/m^2 to correspond to an ambient illumation of 64 lux", which
match the illuminant assumptions for sRGB.""")

JCh_to_XYZ = np.PyUFunc_FromFuncAndData(JCh_to_XYZ_loops,
                                        JCh_to_XYZ_data,
                                        JCh_to_XYZ_types,
                                        1, # number of loops
                                        3, # nin
